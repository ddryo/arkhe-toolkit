// console.log('[Arkhe Toolkit] on Prefetch');
...
function setPrefetch() {
	...
	const addUrlToQueue = (url, processImmediately = false) => {
		...
		// 無視リストと照らし合わせる。
		for (let i = 0; i < window.arkhePrefetchConfig.ignoreKeywords.length; i++) {
			if (url.includes(window.arkhePrefetchConfig.ignoreKeywords[i])) {
	...
	const linksObserver = new IntersectionObserver((entries) => {
		entries.forEach((entry) => {
			if (entry.isIntersecting) {
				...
				addUrlToQueue(url, !window.arkhePrefetchConfig.maxRPS);
			}
		});
	});
	...
	const startQueue = () =>
		setInterval(() => {
			Array.from(toPrefetch)
				.slice(0, window.arkhePrefetchConfig.maxRPS)
				.forEach((url) => {
					prefetchWithTimeout(url);
	...
	const mouseOverListener = (event) => {
		...
		if (elm && elm.href && !alreadyPrefetched.has(elm.href)) {
			...
			}, window.arkhePrefetchConfig.hoverDelay);
		}
	};
	...
	// arkhePrefetchConfigがセットされなかった時のデフォルト値
	...
	window.arkhePrefetchConfig = Object.assign(defaultOptions, window.arkhePrefetchConfig);
	...
	requestIdleCallback(() =>
		setTimeout(
			...
			window.arkhePrefetchConfig.delay * 1000
		)
	);
